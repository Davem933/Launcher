...
**`isNavigating` je jen zrcadlo stavu trip session, ne vlastní flag.** Seed při kompozici z `mapboxNavigation.getTripSessionState() == STARTED` (trip session žije nad kompozicí, na Activity, zatímco `MapWidget` se při přepnutí `MapNavPanel`u Mapa→Navigace celý disposuje — bez toho by se po návratu zpět přes běžící navigaci vykreslilo volné UI; `DisposableEffect` běží až po prvním frame, takže seed je pořád potřeba). Dál to drží **`TripSessionStateObserver`** registrovaný ve stejném `DisposableEffect`u jako `LocationObserver`/`RouteProgressObserver` — ten pokryje i konec navigace *zvenčí* kompozice (dojezd, viz níže). `registerStateObserver` vystřelí aktuální stav hned při registraci a `start()`/`stop()` notifikují observery **synchronně** (ověřeno v `MapboxTripSession.kt` v3.30.1), takže `endGuidance()` (tlačítko Ukončit) i `startTripSession()` volají jen SDK a `isNavigating` si **nesahají ručně** — žádná druhá, paralelní cesta k témuž stavu. Observer taky nuluje `currentManeuvers`/`currentTripProgress` při zastavení, aby další navigace neproblikla banner z té předchozí.
...
**Příjezd ukončí navigaci sám — `data/navigation/MapboxArrivalTeardownObserver.kt`, registrovaný v `CarLauncherApp.onCreate()` přes `MapboxNavigationApp.registerObserver(...)`, tedy stejně jako hlas a NE v `MapWidget`u.** Volá `stopTripSession()` + `setNavigationRoutes(emptyList())`; bez toho by na trvale zapnutém launcheru běžela foreground služba + GPS + hlas donekonečna. **Proč app-level a ne v kompozici:** `MapWidget` při panelu Navigace neexistuje, takže tam by žádný `ArrivalObserver` registrovaný nebyl — a Mapboxí `ArrivalProgressObserver` si dojezd latchuje (`routeArrived = routeProgress.navigationRoute`, zdroják v3.30.1), takže `onFinalDestinationArrival` padne pro danou trasu **jen jednou** a observer registrovaný později už nedostane nic. Dojezd v okně s panelem Navigace by se tím ztratil natrvalo. `MapWidget` na to reaguje čistě přes `TripSessionStateObserver` (viz výše) — **druhý `ArrivalObserver` v kompozici tam schválně není**, aby nebyly dvě cesty, které můžou `stopTripSession()` zavolat dvakrát. `onWaypointArrival` / `onNextRouteLegStart` jsou schválně prázdné (mezicíl nesmí ukončit trasu; `navigateNextRouteLeg()` appka nevolá). **Limit:** `onDetached` přijde i při odchodu appky do pozadí, takže dojezd s launcherem kompletně na pozadí se pořád neodchytí (stejný limit jako u hlasu).
